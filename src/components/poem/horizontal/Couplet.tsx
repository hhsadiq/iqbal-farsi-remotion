...
export const Couplet: React.FC<coupletCompSchema> = ({
	...
	const verseRelativeStartFrame =
		(couplet.verseStartTime - couplet.coupletStartTime) * fps;
	const verseRelativeEndFrame =
		(couplet.verseEndTime - couplet.coupletStartTime) * fps;

	// Typewriter effect for Persian verses
	const charsShownPersian1 =
		frame > verseRelativeStartFrame
			? Math.floor((frame - verseRelativeStartFrame) / 6)
			: 0;
	const textToShowPersian1 = couplet.persian1.slice(0, charsShownPersian1);
	const charsShownPersian2 =
		frame > verseRelativeEndFrame
			? Math.floor((frame - verseRelativeEndFrame) / 6)
			: 0;
	...
	const isTypingSecondVerse =
		frame > verseRelativeEndFrame && !isSecondVerseComplete;
	...
	return (
		<div className="flex flex-row w-full h-full bg-white">
			...
			<div className="w-[70%] flex flex-col mt-40">
				...
				{/* Urdu Translation */}
				<div className="flex items-top justify-center w-full rtl urdu urdu-couplet pt-8 px-16">
					<p className="text-center">{couplet.urdu1}</p>
				</div>
				<div className="flex items-top justify-center w-full px-44 rtl urdu2 urdu-couplet">
					<p className="text-center">{couplet.urdu2}</p>
				</div>
			...
			<div className="w-[30%] flex flex-col items-center justify-center">
				{/* Logo Area */}
				<div
					className="w-[158%] flex flex-col mt-[-132px] justify-start"
					style={{height: '90%'}}
				>
					<div className="w-full">
						<Img
							src={staticFile(globalSettings.logo.horizontal.img)}
							placeholder="logo"
							className="w-full h-auto ml-[-96px]"
						/>
					</div>
				</div>
